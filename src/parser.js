class Flowol {
    resolveFlow(flowchart, mimic, cb) {
                    ...
                    switch (thread.type) {
                        ...
                        case "output":
                            thread.property.forEach((property, i) => {
                                ...
                                if (!checkFormat(value) && !(typeof value === "object" && value.hasOwnProperty("var"))) {
                                    error("Cannot set property '" + property + "' to illegal value type '" + (typeof value) + "'");
                                }

                                if (outputs[property] !== undefined) {
                                    outputs[property] = typeof value === "object" ? vars[value['var']] : value;
                                } else {
